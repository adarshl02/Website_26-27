...
export const MeteorsPremium = ({ number = 20, className }) => {
  const meteors = new Array(number).fill(true);
    ...
    <>
      {meteors.map((_, idx) => (
        <span
          key={"meteor" + idx}
          className={cn(
            "animate-meteor-effect absolute top-1/2 left-1/2 h-0.5 w-0.5 rounded-full bg-yellow-300 shadow-[0_0_0_1px_#ffffff10] rotate-[215deg]",
            "before:content-[''] before:absolute before:top-1/2 before:-translate-y-1/2 before:w-[50px] before:h-[1px] before:bg-gradient-to-r before:from-[#FCD34D] before:to-transparent",
            className
          )}
          style={{
            top: 0,
